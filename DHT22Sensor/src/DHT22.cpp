...
int16_t DHT22::TemperatureAsCelsius()
{
  if (!read())
    return BAD_TEMP;
...
int16_t DHT22::TemperatureAsFarenheit()
{
...
int16_t DHT22::CelsiusToFarenheit(int16_t celsius) const
{
...
bool DHT22::read()
{
  ...
  if (!m_isFirstReading && ((currentTime - m_lastReadTime) < 2000))
    return true; // use the last value if the sensor is probed more often than once / 2 seconds
